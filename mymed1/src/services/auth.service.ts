...
export class AuthService {
  ...
  public async createRefreshToken(user: User): Promise<string> {
    ...
    return redis.setEx(refreshToken, REFRESH_TOKEN_TTL * 365 * 24 * 60 * 60, user.id.toString()).then(() => refreshToken);
  }
  ...
  public blockJWTToken(token: string, value: number, ttl: number): Promise<void> {
    return redis.setEx(token, ttl, value.toString());
  }
}
